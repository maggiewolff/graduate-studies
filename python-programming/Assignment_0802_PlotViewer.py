# Name: Maggie Wolff
# Due Date: 3/4/20
# Assignment 0802 Plot Viewer
# I have not given or received any unauthorized assistance on this assignment.
# Video Link:  https://youtu.be/HY5yCCXHy6A
...
class PlotViewer:
    'view and controller for plot generator'

    def __init__ (self):
        'initialize'

    def registerPlotGenerator(self, generator):
        'registers the plot generator'
        self.pg = generator

        # register the viewer with the generator
        self.pg.registerViewer(PlotViewer())

        return (self.pg)

    def generate(self):
        'display plot'
        self.plot = self.pg.generate()
        return (self.plot)

    def userSelect(self, options, topic):
        'prompts the user to pick one of the options'

        print('\nPlot Viewer Plot Generator:\n')

        count = 1
        print('Choose a ' + topic + ' from the list:')
        for i in options:
            print(count,')',i)
            count += 1
